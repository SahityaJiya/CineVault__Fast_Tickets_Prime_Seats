...
export default async function MovieDetailsPage({ params, searchParams }: MoviePageProps) {
  ...
  const currentCitySlug = sParams.city || 'mumbai';
  const selectedDateStr = sParams.date || new Date().toISOString().split('T')[0];
        ...
        <div className="space-y-6">
          ...
          {theaters.length === 0 && (
            <div className="py-16 text-center rounded-2xl border border-zinc-800 bg-zinc-900/20">
              ...
              <p className="text-zinc-400 text-sm">No showtimes scheduled for this date.</p>
            </div>
          )}
        </div>
      </main>
